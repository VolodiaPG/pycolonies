...
def colonies_client(native_crypto=False):
    ...
    if colonies_tls == "true":
        client = Colonies(
            colonies_server, int(colonies_port), True, native_crypto=native_crypto
        )
    else:
        client = Colonies(
            colonies_server, int(colonies_port), False, native_crypto=native_crypto
        )
...
def func_spec(
    ...
    if isinstance(func, str):
        func_spec = FuncSpec(
            ...
            conditions=Conditions(colonyname=colonyname, executortype=executortype),
        )
        if code is not None:
...
class Colonies:
    ...
    def wait(self, process: Process, timeout, prvkey) -> Process:
        ...
        rpcmsg = {"payloadtype": msg["msgtype"], "payload": "", "signature": ""}

        rpcmsg["payload"] = str(
            base64.b64encode(json.dumps(msg).encode("utf-8")), "utf-8"
        )
        crypto = Crypto()
        ...
        if self.tls:
            ws = create_connection(
                "wss://" + self.host + ":" + str(self.port) + "/pubsub"
            )
        else:
            ws = create_connection(
                "ws://" + self.host + ":" + str(self.port) + "/pubsub"
            )
    ...
    def add_colony(self, colony, prvkey):
        msg = {"msgtype": "addcolonymsg", "colony": colony}
        return self.__rpc(msg, prvkey)

    def del_colony(self, colonyname, prvkey):
        msg = {"msgtype": "removecolonymsg", "colonyname": colonyname}
        return self.__rpc(msg, prvkey)
    ...
    def get_colony(self, colonyname, prvkey):
        msg = {"msgtype": "getcolonymsg", "colonyname": colonyname}
        return self.__rpc(msg, prvkey)

    def add_executor(self, executor, prvkey):
        msg = {"msgtype": "addexecutormsg", "executor": executor}
        return self.__rpc(msg, prvkey)

    def list_executors(self, colonyname, prvkey):
        msg = {"msgtype": "getexecutorsmsg", "colonyname": colonyname}
        return self.__rpc(msg, prvkey)
    ...
    def submit_func_spec(self, spec: FuncSpec, prvkey) -> Process:
        msg = {"msgtype": "submitfuncspecmsg", "spec": spec.model_dump(by_alias=True)}
    ...
    def get_process(self, processid, prvkey) -> Process:
        msg = {"msgtype": "getprocessmsg", "processid": processid}
        response = self.__rpc(msg, prvkey)
        return Process(**response)

    def remove_process(self, processid, prvkey):
        msg = {"msgtype": "removeprocessmsg", "processid": processid}
        return self.__rpc(msg, prvkey)

    def close(self, processid, output, prvkey):
        msg = {"msgtype": "closesuccessfulmsg", "processid": processid, "out": output}

        return self.__rpc(msg, prvkey)

    def fail(self, processid, errors, prvkey):
        msg = {"msgtype": "closefailedmsg", "processid": processid, "errors": errors}

        return self.__rpc(msg, prvkey)

    def set_output(self, processid, arr, prvkey):
        msg = {"msgtype": "setoutputmsg", "processid": processid, "out": arr}

        return self.__rpc(msg, prvkey)

    def stats(self, colonyname, prvkey):
        msg = {"msgtype": "getcolonystatsmsg", "colonyname": colonyname}
        return self.__rpc(msg, prvkey)
    ...
    def add_attribute(self, processid, key, value, prvkey):
        ...
        msg = {"msgtype": "addattributemsg", "attribute": attribute}
        return self.__rpc(msg, prvkey)

    def get_attribute(self, attributeid, prvkey):
        msg = {"msgtype": "getattributemsg", "attributeid": attributeid}
        return self.__rpc(msg, prvkey)

    def get_processgraph(self, processgraphid, prvkey):  # TODO: unittest
        msg = {"msgtype": "getprocessgraphmsg", "processgraphid": processgraphid}
    ...
    def add_function(self, colonyname, executorname, funcname, prvkey):
        ...
        msg = {"msgtype": "addfunctionmsg", "fun": func}
        return self.__rpc(msg, prvkey)
    ...
    def get_functions_by_colony(self, colonyname, prvkey):
        msg = {"msgtype": "getfunctionsmsg", "colonyname": colonyname}
        return self.__rpc(msg, prvkey)
    ...
    def add_log(self, processid, logmsg, prvkey):
        msg = {"msgtype": "addlogmsg", "processid": processid, "message": logmsg}
        return self.__rpc(msg, prvkey)
    ...
    def get_files(self, label, colonyname, prvkey):
        msg = {"msgtype": "getfilesmsg", "colonyname": colonyname, "label": label}
        return self.__rpc(msg, prvkey)

    def add_cron(
        self, cronname, cronexpr, wait, workflow: Workflow, colonyname, prvkey
    ):
        ...
        msg = {"msgtype": "addcronmsg", "cron": cron}
        return self.__rpc(msg, prvkey)

    def get_cron(self, cronid, prvkey):
        msg = {"msgtype": "getcronmsg", "cronid": cronid}
        return self.__rpc(msg, prvkey)

    def get_crons(self, colonyname, count, prvkey):
        msg = {"msgtype": "getcronsmsg", "colonyname": colonyname, "count": count}
        return self.__rpc(msg, prvkey)

    def del_cron(self, cronid, prvkey):
        msg = {"msgtype": "removecronmsg", "all": False, "cronid": cronid}
        return self.__rpc(msg, prvkey)
    ...
    def upload_file(
        self, colonyname, prvkey, filepath=None, label=None, metadata_endpoint=None
    ):
    ...
    def __upload_file(
        ...
        if metadata_endpoint is not None:
            endpoint_parts = metadata_endpoint.split(":")
        else:
        ...
        try:
            if file_bytes is None:
                ...
                s3_client.put_object(
                    Bucket=bucket_name, Key=object_name, Body=file_bytes
                )
        except Exception as e:
            raise e
        ...
        ref = Reference(protocol="s3", s3object=obj)
        ...
        msg = {"msgtype": "addfilemsg", "file": f.model_dump(by_alias=True)}

        return self.__rpc(msg, prvkey)

    def get_file(
        self, colonyname, prvkey, label=None, fileid=None, filename=None, latest=True
    ):
        if fileid is not None and filename is not None:
            raise ValueError(
                "Both 'fileid' and 'filename' cannot be set at the same time. Please provide only one."
            )
    ...
    def __remove_file(self, label, fileid, name, colonyname, prvkey):
        if fileid is not None and name is not None:
            raise ValueError(
                "Both 'fileid' and 'name' cannot be set at the same time. Please provide only one."
            )
    ...
    def download_file(
        ...
        endpoint = (
            file[0]["ref"]["s3object"]["server"]
            + ":"
            + str(file[0]["ref"]["s3object"]["port"])
        )
    ...
    def delete_file(self, colonyname, prvkey, label=None, fileid=None, filename=None):
        ...
        file = self.get_file(
            colonyname, prvkey, label=label, fileid=fileid, filename=filename
        )
